 ...
 * It injects the statusline as additional context.
 */
...
async function main(): Promise<void> {
  try {
    ...
    const eventName = input.eventName || "SessionStart";
    ...
    // Also log to stderr for debugging (visible in Claude Code logs)
    const compact = formatCompactStatusline(gitInfo, tokenUsage, config);
    console.error(`[Statusline] ${compact}`);
  } catch (error) {
    // Log error but don't fail the hook
    console.error(`[Statusline] Error: ${error}`);

    // Return empty output to allow session to continue
    await writeOutput({
      hookSpecificOutput: {
