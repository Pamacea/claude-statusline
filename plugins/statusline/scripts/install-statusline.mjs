...
function updateSettings() {
  ...
  if (existsSync(SETTINGS_PATH)) {
    try {
      ...
      log('! Settings file may be corrupted or in an unexpected format.', 'warn');
      hadParseError = true;
      // Don't proceed if we can't parse existing settings
      log('! Skipping automatic settings update to avoid data loss.', 'warn');
      log('! Please add the following manually to ~/.claude/settings.json:', 'warn');
