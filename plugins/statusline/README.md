...
After installing this plugin from the marketplace:

1. **Run the setup command:**
   ```
   /setup
   ```
   This will install the statusline script and configure your `~/.claude/settings.json`.

2. **Restart Claude Code** for the statusline to appear.

### Manual Installation

If the `/setup` command doesn't work, run manually:

```bash
# From the plugin repository root
node plugins/statusline/scripts/install-statusline.mjs
```
